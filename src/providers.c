...
static const int PROVIDER_COUNT =
    (int)(sizeof PROVIDERS / sizeof PROVIDERS[0]);
...
{
    if (!name) return PROVIDER_UNKNOWN;
    if (strcmp(name, "all") == 0) return PROVIDER_ALL;
...
{
    for (int i = 0; i < PROVIDER_COUNT; i++) {
        if (PROVIDERS[i].id == p) return PROVIDERS[i].name;
    }
    return "unknown";
}
...
void list_providers(void)
{
    ...
    for (int i = 0; i < PROVIDER_COUNT; i++) {
        printf("  " COL_CYAN "%-20s" COL_RESET "  %s\n",
               PROVIDERS[i].slug, PROVIDERS[i].name);
    }
...
void print_result(const SpeedResult *r,
                  const PingResult  *pr,
                  const ConnResult  *cr)
{
    if (!r->valid) {
    ...
    if (r->city[0])    printf(COL_BOLD "  %-12s" COL_RESET " %s\n", "City:",    r->city);
    if (r->region[0])  printf(COL_BOLD "  %-12s" COL_RESET " %s\n", "Region:",  r->region);
    if (r->country[0]) printf(COL_BOLD "  %-12s" COL_RESET " %s\n", "Country:", r->country);
    if (r->org[0])     printf(COL_BOLD "  %-12s" COL_RESET " %s\n", "Org/ISP:", r->org);
    ...
    if (cr && cr->success) {
        const char *col = cr->ttfb_ms < 200 ? COL_GREEN
                        : cr->ttfb_ms < 600 ? COL_YELLOW : COL_RED;
    ...
    if (r->upload_mbps >= 0.0)
        printf(COL_BOLD "  %-12s" COL_RESET COL_MAGENTA " %.1f Mbps\n" COL_RESET,
...
SpeedResult run_provider(Provider p)
{
    ...
    for (int i = 0; i < PROVIDER_COUNT; i++) {
        if (PROVIDERS[i].id == p) { def = &PROVIDERS[i]; break; }
    }
    if (!def) {
