...
static void usage(const char *argv0)
{
    printf(
        "\n"
        COL_BOLD "speedtest-cli" COL_RESET " - open-source network speed tester\n"
        "\n"
        "Usage:\n"
        "  %s [options]\n"
        "\n"
        "Options:\n"
        "  " COL_CYAN "--provider=<slug>" COL_RESET "   Provider to query  (default: all)\n"
        "  " COL_CYAN "--list" COL_RESET "               List available providers and exit\n"
        "  " COL_CYAN "--ping-only" COL_RESET "          Only run TCP-connect ping test\n"
        "  " COL_CYAN "--json" COL_RESET "               Output results as JSON\n"
        "  " COL_CYAN "--count=<n>" COL_RESET "          Ping probes per host (default: 4)\n"
...
static void print_json_result(const SpeedResult *r,
                               const PingResult  *pr,
                               const ConnResult  *cr)
{
    ...
    if (cr) {
        ...
        cJSON_AddNumberToObject(obj, "ttfb_ms",   cr->success ? cr->ttfb_ms  : -1.0);
        cJSON_AddNumberToObject(obj, "total_ms",  cr->success ? cr->total_ms : -1.0);
...
typedef struct { const char *slug; const char *host; } PingEntry;
...
static void run_ping_only(Provider p, int count, int json_mode)
{
    for (int i = 0; i < PING_TABLE_LEN; i++) {
        ...
        if (p != PROVIDER_ALL && pid != p) continue;
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg, "--ping-only") == 0) { ping_only = 1; continue; }
        if (strcmp(arg, "--json")      == 0) { json_mode = 1; continue; }
    ...
    if (chosen == PROVIDER_ALL) {
        for (int p = PROVIDER_IPAPI; p < PROVIDER_ALL; p++) {
            ...
            if (json_mode) {
                PingResult pr; memset(&pr, 0, sizeof pr);
                ConnResult cr; memset(&cr, 0, sizeof cr);
